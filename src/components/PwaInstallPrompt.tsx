...
export default function PwaInstallPrompt() {
  ...
  useEffect(() => {
    ...
    if (isIosDevice) {
      // Show iOS specific installation instructions if not in standalone
      const hasDismissedIos = localStorage.getItem("dismissed_pwa_ios");
      if (!hasDismissedIos) {
        setShowPrompt(true);
      }
    }
  ...
  const handleDismiss = () => {
    setShowPrompt(false);
    if (isIos) {
      localStorage.setItem("dismissed_pwa_ios", "true");
    }
  };
