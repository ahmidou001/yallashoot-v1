        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {articles.map((article: any, index: number) => {
              const showNativeAdHere = articles.length >= 5 ? index === 4 : index === articles.length - 1;
              ...
              return (
                <React.Fragment key={article._id}>
                  ...
                  {/* Native In-Feed Article Ad Card (Matches Image 4 circled element) */}
                  {showNativeAdHere && (
                    <div
                      key="news-native-ad"
                      className="flex flex-col overflow-hidden rounded-2xl bg-zinc-900 border border-zinc-800 p-4 text-center shadow-lg justify-between min-h-[320px]"
                    >
                      ...
                      <div className="flex items-center justify-between text-[10px] text-zinc-550 font-bold select-none border-t border-zinc-850 pt-2.5">
                        <span className="text-zinc-400 font-bold">يلا شوت نيوز</span>
                        <span className="text-emerald-400 font-bold">تغطية حصرية</span>
                      </div>
                    </div>
                  )}
                </React.Fragment>
